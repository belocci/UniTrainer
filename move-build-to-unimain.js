...
const tempBuildDir = path.join(__dirname, 'build-output', 'Uni Trainer-win32-x64');
const targetDir = __dirname; // UNIMAIN directory
...
try {
  ...
  try {
    ...
    fs.rmSync(path.join(__dirname, 'build-output'), { recursive: true, force: true });
  } catch (rmError) {
